document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch and parse JSON data
    // const dataPath = 'DATA/vehicles.json'; // Removed fetch

    // FIX: Use window.VEHICLE_DATA directly
    try {
        if (!window.VEHICLE_DATA) {
             throw new Error("Vehicle data script (vehicles_data.js) not loaded.");
        }
        ...
        const fixPath = (p) => {
            ...
            if (p.startsWith("images/Car images/")) {
                // Phone version is in phone/category.html
                // Image folder is in root/Sherif-Bach/Car images
        ...
        allData = data.map(item => ({
            brand: item.brand,
            model: item.model,
            year: item.year || item.yearStart, // Use year directly if available
            type: item.type || (item.specs ? item.specs.bodyType : "Car"),
            id: item.id,
            // Add image paths to object for rendering
            image: fixPath(item.coverImage || (item.images ? item.images[0] : null)),
            realImagePath: fixPath(item.images ? item.images.find(i=>i.includes('menu.png')) : null) 
        }));
        ...
        pageData.forEach((item, index) => {
            ...
            card.innerHTML = `
                <div class="card-header">
                 ...
                 <div class="card-image-wrapper" style="height: 150px; overflow: hidden; border-radius: 8px; margin: 0.5rem 0;">
                     <img src="${item.realImagePath || item.image}" alt="${item.model}" style="width: 100%; height: 100%; object-fit: cover;" loading="lazy" onerror="this.onerror=null; this.src='https://placehold.co/600x400?text=${item.model}';">
                 </div>
                <h3 class="model-name">${highlightedModel}</h3>
                <div class="card-footer">
                    <button class="view-btn" onclick="location.href='image-preview.html?id=${item.id}'">
                        View Options <i class="fas fa-arrow-right"></i>
                    </button>
                </div>
            `;
